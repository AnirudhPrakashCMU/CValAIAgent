...
class TranscriptMsgPayload(AppBaseModel):
    ...
    msg_id: UUID = Field(
        ...
        description="Unique ID for this specific transcript message/segment.",
    )
    utterance_id: UUID = Field(
        description="ID for the continuous utterance this segment belongs to."
    )
    text: str
    ts_start: float = Field(
        description="Timestamp of the start of this segment in seconds, relative to utterance start."
    )
    ts_end: float = Field(
        description="Timestamp of the end of this segment in seconds, relative to utterance start."
    )
...
class DesignSpecPayload(AppBaseModel):
    ...
    source_utts: List[UUID] = Field(
        default_factory=list,
        description="List of utterance_ids that contributed to this spec.",
    )
...
class InsightMsgPayload(AppBaseModel):
    ...
    demographic_breakdown: Dict[
        str, Dict[Literal["positive", "neutral", "negative"], int]
    ]  # e.g. {"Gen Z": {"positive": 10, ...}}
...
class WSServiceStatusMessage(AppBaseModel):
    ...
    kind: Literal["service_status"] = (
        "service_status"  # Changed from "service_down" to be more general
    )
    service_name: str
...
if __name__ == "__main__":
    ...
    client_edit_example = ClientEditComponentMessage(
        session_id=uuid4(), spec_id=uuid4(), code="<button>Updated Button</button>"
    )
