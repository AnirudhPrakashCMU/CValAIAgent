...
class ClientConnection:
    ...
    def __init__(self, websocket: WebSocket, session_id: str):
        ...
        self.client_id = f"{websocket.client.host}:{websocket.client.port}" if websocket.client else f"unknown-{uuid.uuid4()}"
        self.outgoing_queue: asyncio.Queue[str] = asyncio.Queue(maxsize=settings.WEBSOCKET_MAX_QUEUE_SIZE)
    ...
    async def send_json_str(self, json_str: str):
        ...
        if not self.active:
            logger.warning(f"[{self.client_id}] Attempted to queue message for inactive connection. Session: {self.session_id}")
            return
        try:
            ...
            # TODO: Implement more sophisticated backpressure or disconnect client if consistently full.

    async def close(self, code: int = status.WS_1000_NORMAL_CLOSURE, reason: Optional[str] = None):
        """Gracefully closes the WebSocket connection and cancels associated tasks."""
        if not self.active:
            return
        self.active = False
        logger.info(f"[{self.client_id}] Closing connection for session {self.session_id}. Code: {code}, Reason: {reason}")
        ...
        if self.websocket.client_state != self.websocket.client_state.DISCONNECTED:
            try:
                ...
                logger.debug(f"[{self.client_id}] Error closing WebSocket (likely already closed): {e}")
            except Exception as e:
                logger.error(f"[{self.client_id}] Unexpected error closing WebSocket: {e}", exc_info=True)
        logger.info(f"[{self.client_id}] Connection resources cleaned up for session {self.session_id}.")
...
class ConnectionManager:
    ...
    async def connect(self, client: ClientConnection):
        ...
        logger.info(f"Client {client.client_id} (Session: {client.session_id}) connected. Total active: {len(self.active_connections)}")
    ...
    async def disconnect(self, client: ClientConnection):
        ...
        logger.info(f"Client {client.client_id} (Session: {client.session_id}) disconnected. Total active: {len(self.active_connections)}")
    ...
    async def broadcast(self, message_json_str: str):
        ...
        logger.debug(f"Broadcasting message to {len(connections_to_send)} clients: {message_json_str[:100]}...")
...
async def global_redis_message_handler(channel_name: str, data_bytes: bytes):
    ...
    try:
        ...
        #     outgoing_message = WSServiceStatusMessage(**payload_dict)
            ...
            logger.warning(f"Received message from unmapped Redis channel '{channel_name}': {message_str[:200]}")
            return

    except json.JSONDecodeError:
        logger.error(f"Failed to decode JSON from Redis channel '{channel_name}': {message_str[:200] if message_str else 'Empty data'}", exc_info=True)
        return
    except UnicodeDecodeError:
        logger.error(f"Failed to decode UTF-8 from Redis channel '{channel_name}'. Data (hex): {data_bytes.hex()[:100]}", exc_info=True)
        return
    except Exception as e: # Catch Pydantic validation errors or other issues
        logger.error(f"Error processing message from Redis channel '{channel_name}': {e}. Original data: {message_str[:200] if message_str else data_bytes.hex()[:100]}", exc_info=True)
        return
    ...
    if outgoing_message:
        try:
            ...
            logger.error(f"Error serializing or broadcasting outgoing WebSocket message: {e}", exc_info=True)


async def _websocket_sender_task(client: ClientConnection):
    """Sends messages from the client's outgoing queue to the WebSocket."""
    logger.info(f"[{client.client_id}] Sender task started for session {client.session_id}.")
    try:
        while client.active:
            try:
                message_json_str = await asyncio.wait_for(client.outgoing_queue.get(), timeout=1.0)
                if client.active and client.websocket.client_state == client.websocket.client_state.CONNECTED:
                    await client.websocket.send_text(message_json_str)
                    client.outgoing_queue.task_done()
                    logger.debug(f"[{client.client_id}] Sent message to session {client.session_id}: {message_json_str[:100]}...")
                else:
                    logger.warning(f"[{client.client_id}] WebSocket not connected or client inactive; cannot send. Message requeued or dropped if queue full.")
            ...
            except WebSocketDisconnect: # Should be caught by receiver, but good to handle here too
                logger.info(f"[{client.client_id}] WebSocket disconnected during send. Session: {client.session_id}")
                await manager.disconnect(client)
                break
            except Exception as e:
                logger.error(f"[{client.client_id}] Error in sender task for session {client.session_id}: {e}", exc_info=True)
                # Depending on error, might need to disconnect client
                await manager.disconnect(client) # Disconnect on unknown send error
                break
    except asyncio.CancelledError:
        logger.info(f"[{client.client_id}] Sender task cancelled for session {client.session_id}.")
    finally:
        logger.info(f"[{client.client_id}] Sender task stopped for session {client.session_id}.")


async def _websocket_receiver_task(client: ClientConnection):
    """Receives messages from the WebSocket client and processes them."""
    logger.info(f"[{client.client_id}] Receiver task started for session {client.session_id}.")
    try:
        while client.active:
            try:
                ...
                message_text = await asyncio.wait_for(client.websocket.receive_text(), timeout=settings.WEBSOCKET_HEARTBEAT_INTERVAL_S + 5.0)
                logger.debug(f"[{client.client_id}] Received message from client (Session {client.session_id}): {message_text[:100]}...")

                # Attempt to parse as a known client message type
                try:
                    data_dict = json.loads(message_text)
                    kind = data_dict.get("kind")
                    parsed_message: Optional[OrchestratorWebSocketIncomingMessage] = None

                    if kind == "audio_chunk":
                        # parsed_message = ClientAudioChunkMessage(**data_dict)
                        # TODO: Handle audio_chunk: decode base64, forward to STT service.
                        # This requires Orchestrator to act as a WebSocket client to STT.
                        logger.info(f"[{client.client_id}] Received 'audio_chunk' from client (Session {client.session_id}). Length: {len(data_dict.get('data_b64', ''))}. TODO: Implement proxying.")
                    elif kind == "edit_component":
                        # parsed_message = ClientEditComponentMessage(**data_dict)
                        # TODO: Handle component edit requests.
                        logger.info(f"[{client.client_id}] Received 'edit_component' from client (Session {client.session_id}). Spec ID: {data_dict.get('spec_id')}. TODO: Implement edit handling.")
                    elif kind == "control_session":
                        # parsed_message = ClientControlMessage(**data_dict)
                        # TODO: Handle session control messages.
                        logger.info(f"[{client.client_id}] Received 'control_session' from client (Session {client.session_id}). Action: {data_dict.get('action')}. TODO: Implement control handling.")
                    elif kind == "ping_custom": # Example custom ping
                        await client.send_json_str(WSServiceStatusMessage(kind="service_status", service_name="orchestrator", status="up", message="pong_custom").model_dump_json())

                    else:
                        logger.warning(f"[{client.client_id}] Received unknown message kind '{kind}' from client (Session {client.session_id}).")

                    # if parsed_message:
                    #     logger.info(f"Parsed client message: {parsed_message}")

                except json.JSONDecodeError:
                    logger.warning(f"[{client.client_id}] Received non-JSON message from client (Session {client.session_id}): {message_text[:200]}")
                except Exception as e_parse: # Pydantic validation error etc.
                    logger.error(f"[{client.client_id}] Error parsing client message (Session {client.session_id}): {e_parse}. Original: {message_text[:200]}", exc_info=True)
                ...
                if not client.active:
                    break
                logger.debug(f"[{client.client_id}] Timeout waiting for client message. Session: {client.session_id}. Still active.")
                continue
            except WebSocketDisconnect:
                logger.info(f"[{client.client_id}] WebSocket disconnected by client (receiver task). Session: {client.session_id}")
                await manager.disconnect(client)
                break # Exit loop
            except Exception as e:
                logger.error(f"[{client.client_id}] Error in receiver task for session {client.session_id}: {e}", exc_info=True)
                await manager.disconnect(client) # Disconnect on unknown error
                break
    except asyncio.CancelledError:
        logger.info(f"[{client.client_id}] Receiver task cancelled for session {client.session_id}.")
    finally:
        logger.info(f"[{client.client_id}] Receiver task stopped for session {client.session_id}.")
...
async def _websocket_heartbeat_task(client: ClientConnection):
    ...
    logger.info(f"[{client.client_id}] Heartbeat task started for session {client.session_id}.")
    try:
        while client.active:
            ...
            if client.active and client.websocket.client_state == client.websocket.client_state.CONNECTED:
                try:
                    ...
                    logger.debug(f"[{client.client_id}] Heartbeat check for session {client.session_id}. (Relying on Uvicorn keepalive)")
                except WebSocketDisconnect:
                    logger.info(f"[{client.client_id}] WebSocket disconnected during heartbeat. Session: {client.session_id}")
                    await manager.disconnect(client)
                    break
                except Exception as e:
                    logger.warning(f"[{client.client_id}] Error sending heartbeat for session {client.session_id}: {e}")
            else:
                logger.info(f"[{client.client_id}] Client no longer active or WebSocket disconnected; stopping heartbeat. Session: {client.session_id}")
                break
    except asyncio.CancelledError:
        logger.info(f"[{client.client_id}] Heartbeat task cancelled for session {client.session_id}.")
    finally:
        logger.info(f"[{client.client_id}] Heartbeat task stopped for session {client.session_id}.")


@router.websocket("/v1/ws/{session_id}")
async def websocket_endpoint(
    websocket: WebSocket,
    session_id: str = Path(..., description="Unique identifier for the client session."),
    # token: Optional[str] = Query(None, description="JWT token for authentication"), # TODO: Add authentication
):
    """
    ...
    """
    # TODO: Implement JWT authentication using the 'token' query parameter.
    # For now, we proceed without authentication for simplicity.
    # if not token:
    #     await websocket.close(code=status.WS_1008_POLICY_VIOLATION, reason="Missing authentication token")
    #     return
    # try:
    #     payload = security.decode_jwt_token(token) # Assuming a security utility
    #     # Validate payload, e.g., check if session_id matches or if user has rights
    ...
    try:
        ...
        logger.info(f"[{client.client_id}] WebSocket disconnected (caught in endpoint). Session: {client.session_id}")
    except Exception as e:
        logger.error(f"[{client.client_id}] Unexpected error in WebSocket endpoint for session {client.session_id}: {e}", exc_info=True)
        await client.close(code=http_status.WS_1011_INTERNAL_ERROR, reason="Internal server error")
    finally:
        logger.info(f"[{client.client_id}] Cleaning up endpoint for session {client.session_id}.")
        # Disconnect will also ensure tasks are cancelled and connection removed from manager
        await manager.disconnect(client)
        logger.info(f"[{client.client_id}] Endpoint cleanup complete for session {client.session_id}.")
