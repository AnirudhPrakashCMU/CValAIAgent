...
export default function App() {
  ...
  function stopRecording() {
    if (recorderRef.current) {
      ...
      recordingChunksRef.current = [];
      log('Stopped recording, sending audio for API transcription');
  ...
  async function sendForTranscription(blob) {
    ...
    try {
      const res = await fetch(`${HTTP_URL}/v1/transcribe`, { method: 'POST', body: form });
      const data = await res.json();
      if (data.text) {
        setTranscripts((t) => [...t, data.text]);
        log('Transcription received');
      } else {
        log('Transcription response had no text');
      }
    } catch (err) {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {logs.length > 0 && (
        <div>
          <h2 className="font-semibold mt-4 mb-2">Debug Logs</h2>
          <pre className="bg-black text-green-300 p-2 text-xs max-h-40 overflow-auto">
            {logs.join('\n')}
          </pre>
        </div>
      )}
