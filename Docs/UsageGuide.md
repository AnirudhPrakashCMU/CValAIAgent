...
2. Wait for Docker images to build and services to start. The frontend will be available at `http://localhost:5173`.
   The speech‑to‑text image is built for the `linux/amd64` platform so that CPU
   PyTorch wheels are available even on Apple Silicon hosts. Docker Desktop
   handles this automatically via emulation.
